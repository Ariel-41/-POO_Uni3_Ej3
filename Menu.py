...
class Menu:
    ...
    def opcion1(self, unManejadorEquipos:ManejadorEquipos, unManejadorJugadores:ManejadorJugadores, unManejadorContratos:ManejadorContratos):
        ...
        dni = self.__ingresador.inputRegularExpression("Ingrese el DNI sin puntos[Los 8 Numeros] : ", "[0-9]{8,8}", "DNI invalido, reintente: ")
        ciudad = input("Ingrese su ciudad natal: ")
        paisOrigen = input("Ingrese su pais de origen: ")
        fechaNacimiento = self.__ingresador.inputFecha("Ingrese su fecha de nacimiento en el formato dd/mm/aaaa: ")
    ...
    def opcion2(self, unManejadorJugadores:ManejadorJugadores):
        dni = self.__ingresador.inputRegularExpression("Ingrese el DNI del jugador [Los 8 Numeros]: ", "[0-9]{8,8}", "DNI invalido, reintente: ")
        indice = unManejadorJugadores.buscarJugadorDni(dni)
        if indice == -1:
            print("No se encontro el jugador con DNI {0}".format(dni))
        else:
    ...
    def opcion3(self, unManejadorEquipos:ManejadorEquipos):
        ...
        if indice == -1:
            print("No se encuentra un equipo con el nombre {0}".format(nombre))
        else:
            unEquipo = unManejadorEquipos.getEquipo(indice)
    ...
    def opcion4(self, unManejadorEquipos:ManejadorEquipos):
        ...
        indice = unManejadorEquipos.buscarEquipoNombre(nombre)
        if indice == -1:
            print("No se encontro un equipo con el nombre {0}".format(nombre))
        else:
